...
test('check that the Friend component renders properly', async () => {
    const {getByText} = render(<FriendComponent friendUrl ={"a"} friendName = "g" session={[]} webId={"https://uo281835.inrupt.net/profile/card#me"}/>);
    const {getByTestId} = render(<FriendComponent friendUrl ={"a"} friendName = "g" session={[]} webId={"https://uo281835.inrupt.net/profile/card#me"}/>);
